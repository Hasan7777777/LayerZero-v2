...
module endpoint_v2_common::universal_config {
    ...
    public entry fun initialize(account: &signer, eid: u32) {
        assert!(address_of(account) == @endpoint_v2_common, EUNAUTHORIZED);
    ...
    #[test_only]
    public fun init_module_for_test(eid: u32) {
        let account = &create_signer_for_test(@endpoint_v2_common);
        initialize(account, eid);
    }
    ...
    const EINVALID_ZRO_ADDRESS: u64 = 2;
    const ENO_CHANGE: u64 = 3;
    const EZRO_ADDRESS_NOT_SET: u64 = 4;
    const EZRO_ADDRESS_LOCKED: u64 = 5;
}
