module endpoint_v2::channels {
    ...
    /// Nillified packets cannot be burnt.
    public(friend) fun nilify(receiver: address, src_eid: u32, sender: Bytes32, nonce: u64, payload_hash: Bytes32) {
        ...
        // Set the hash to empty string to indicate that the packet is nilified
    ...
    public(friend) fun clear_payload(
        ...
        let expected_hash = store::get_payload_hash(receiver, src_eid, sender, nonce);
        assert!(actual_hash == expected_hash, EPAYLOAD_HASH_DOES_NOT_MATCH);

        // Clear the payload hash
        store::remove_payload_hash(receiver, src_eid, sender, nonce);
